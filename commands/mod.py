...
class mod(commands.Cog):
    ...
    async def unban(self , ctx , user: discord.User = None , * , reason="Kein Grund angegeben"):
        herobrine = 378845487945482243
        otheracchero = 439898683585200131
        maxi = 757689875129434285
        rayz = 632974068911570945
        mario = 586807644753231891
        robin = 780471476910161940
        dustin = 528284722384601110
        vogel_der_nicht_fliegt = 474804209381015553
        if ctx.message.author.id == herobrine or ctx.message.author.id == vogel_der_nicht_fliegt or ctx.message.author.id == otheracchero or ctx.message.author.id == mario or ctx.message.author.id == maxi or ctx.message.author.id == robin or ctx.message.author.id == dustin:
            embed = discord.Embed(timestamp = ctx.message.created_at , color = 0xff0000 ,
                                  title = "Ausgeschlossen!" ,
                                  description = "Du bist nicht dazu berechtigt, diesen Bot zu benutzen!\n\nGrund: Unterstützung von Pädophilie, Verstoß gegen die ToS auf Averge Network sowie Rufmord an die ZAE und GamingEcke")
            embed.set_author(name = f"{self.zeus.user}" , icon_url = f"{self.zeus.user.avatar.url}")
            return await ctx.reply(embed = embed , delete_after = 10)
        elif ctx.message.author.id == rayz:
            embed = discord.Embed(timestamp = ctx.message.created_at , color = 0xff0000 ,
                                  title = "Ausgeschlossen!" ,
                                  description = "Du bist nicht dazu berechtigt, diesen Bot zu benutzen!\n\nGrund: Morddrohung an einen ZAE-Inhaber")
            embed.set_author(name = f"{self.zeus.user}" , icon_url = f"{self.zeus.user.avatar.url}")
            return await ctx.reply(embed = embed , delete_after = 3)
        else:
            if user is None:
    ...
    async def ban(self , ctx , user: Union[discord.Member , discord.User] = None , * , reason="Kein Grund angegeben"):
        ...
        if ctx.message.author.id == herobrine or ctx.message.author.id == vogel_der_nicht_fliegt or ctx.message.author.id == otheracchero or ctx.message.author.id == mario or ctx.message.author.id == maxi or ctx.message.author.id == robin or ctx.message.author.id == dustin:
            embed = discord.Embed(timestamp = ctx.message.created_at , color = 0xff0000 ,
                                  title = "Ausgeschlossen!" ,
                                  description = "Du bist nicht dazu berechtigt, diesen Bot zu benutzen!\n\nGrund: Unterstützung von Pädophilie, Verstoß gegen die ToS sowie Rufmord an die ZAE und GamingEcke")
            embed.set_author(name = f"{self.zeus.user}" , icon_url = f"{self.zeus.user.display_avatar}")
            return await ctx.reply(embed = embed , delete_after = 10)
        elif ctx.message.author.id == rayz:
            embed = discord.Embed(timestamp = ctx.message.created_at , color = 0xff0000 ,
                                  title = "Ausgeschlossen!" ,
    ...
    async def takerole(self , ctx , member: discord.Member = None , * , role: discord.Role = None):
        ...
        if ctx.message.author.id == herobrine or ctx.message.author.id == vogel_der_nicht_fliegt or ctx.message.author.id == otheracchero or ctx.message.author.id == mario or ctx.message.author.id == maxi or ctx.message.author.id == robin or ctx.message.author.id == dustin:
            embed = discord.Embed(timestamp = ctx.message.created_at , color = 0xff0000 ,
                                  title = "Ausgeschlossen!" ,
                                  description = "Du bist nicht dazu berechtigt, diesen Bot zu benutzen!\n\nGrund: Unterstützung von Pädophilie, Verstoß gegen die ToS sowie Rufmord an die ZAE und GamingEcke")
            embed.set_author(name = f"{self.zeus.user}" , icon_url = ctx.author.display_avatar)
            return await ctx.reply(embed = embed , delete_after = 10)
        elif ctx.message.author.id == rayz:
            embed = discord.Embed(timestamp = ctx.message.created_at , color = 0xff0000 ,
                                  title = "Ausgeschlossen!" ,
